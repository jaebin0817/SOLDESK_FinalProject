...
function idCheck(){
	...
	if(mem_id.length < 3 || mem_id == null){
		$("#checkId").html("아이디 3글자 넘어 입력해 주세요");
...
function emailCheck() {
   ...
   if(exptext.test(mem_email)==false){
	   ...
	   $("#checkEmail").html("이메일형식이 올바르지 않습니다.");
	...
	if(p_check == "" || p_check.test(mem_phone) == false){
		...
		$.ajax({
			...
			success : function(result){
				isRun = false;
				if(result == 1){	
					$("#checkPhone").html("사용 가능한 전화번호 입니다.");
					$("#checkPhone").attr('color', 'blue');
					phonecheck = 0;
					check = 0;
					return true;
				}else{
					$("#checkPhone").html("기존에 있는 전화번호 입니다.");
...
function newpwCheck(){
	...
	if(mem_pw == new_pw){
		...
		if(newre_pw != new_pw){
			$("#checknPw").html("신규 비밀번호랑 확인이 다릅니다");
			...
			$("#checknPw").html("신규 비밀번호와 확인이 같습니다");
