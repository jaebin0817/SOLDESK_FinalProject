...
public class ReviewDAO {
   ...
   public ArrayList<ReviewDTO> reviewAll(int mcode) {  // rev_read랑 동일하게봄
      ...
      try {
         ...
         if (rs.next()) {
        	 reviewAllList = new ArrayList<ReviewDTO>(); 
        	 do {
            dto2 = new ReviewDTO();
            dto2.setMcode(mcode);
            ...
            reviewAllList.add(dto2);
         }while (rs.next());
       }// if end
     } catch (Exception e) {
         System.out.println("리뷰 전체 불러오기 실패 : " + e);
      } finally {
   ...
   public ArrayList<ReviewDTO> reviewThree(int mcode) {  // 리뷰 세개만 보여주게 만듦. (contlistread.do에서)
	      ...
	      try {
	        ...
	        do {
	            dto2 = new ReviewDTO();// 커서가 가리키는 한 줄임.
	            dto2.setMcode(mcode);
	            dto2.setRev_code(rs.getInt("rev_code"));
	            dto2.setMem_id(rs.getString("mem_id"));
	            dto2.setRev_title(rs.getString("rev_title"));
	         ...
	         System.out.println("상위 리뷰 3개 불러오기 실패 : " + e);
	      } finally {
	         DBclose.close(con, pstmt, rs);
	      } // try end
